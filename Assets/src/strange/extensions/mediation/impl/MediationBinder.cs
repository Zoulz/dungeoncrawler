...
namespace strange.extensions.mediation.impl
{
	public class MediationBinder : Binder, IMediationBinder
	{
		...
		/// Initialize all IViews within this NavigationView
		...
		virtual protected void mapView(IView view, IMediationBinding binding)
		{
			...
			if (bindings.ContainsKey(viewType))
			{
				...
				for (int a = 0; a < aa; a++)
				{
					...
					if (mediator == null)
						throw new MediationException ("The NavigationView: " + viewType.ToString() + " is mapped to mediator: " + mediatorType.ToString() + ". AddComponent resulted in null, which probably means " + mediatorType.ToString().Substring(mediatorType.ToString().LastIndexOf(".")+1) + " is not a MonoBehaviour.", MediationExceptionType.NULL_MEDIATOR);
		...
		/// Removes a mediator when its NavigationView is destroyed
