...
namespace strange.extensions.mediation.impl
{
	public class View : MonoBehaviour, IView
	{
		...
		/// a NavigationView to exist outside a context you can set it to false. The only
		...
		virtual protected void bubbleToContext(MonoBehaviour view, bool toAdd, bool finalTry)
		{
			...
			if (requiresContext && finalTry)
			{
				...
				string msg = (loopLimiter == LOOP_MAX) ?
					msg = "A NavigationView couldn't find a context. Loop limit reached." :
						msg = "A NavigationView was added with no context. Views must be added into the hierarchy of their ContextView lest all hell break loose.";
				msg += "\nView: " + view.ToString();
